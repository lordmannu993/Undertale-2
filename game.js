...
const rooms = {
  ruins: {
    ...
    name: 'Ruins - Entrance',
    exits: { right: 'hall' },
    npc: { x: 300, y: 220, text: ['* Howdy!', '* This is a tiny one-file recreation attempt.', '* Stay determined.'] },
  },
  hall: {
    id: 'hall',
    name: 'Ruins - Hallway',
...
window.addEventListener('keydown', (e) => {
  ...
  if (e.key.toLowerCase() === 'z') handleConfirm();
  if (e.key.toLowerCase() === 'x') handleBack();
});
...
function handleConfirm() {
  ...
  if (state.mode === 'battle' && state.turn === 'menu') {
    ...
    if (choice === 'FIGHT') {
      ...
      state.dialog = { lines: ['* Not implemented in this prototype.'], index: 0 };
    }
  } else if (state.mode === 'overworld') {
    const npc = state.room.npc;
    if (npc && touching(state.player, { ...npc, w: 20, h: 24 })) {
      state.dialog = { lines: npc.text, index: 0 };
      if (state.room.id === 'battle') startBattle();
    }
  }
}

function handleBack() {
  if (state.mode === 'battle' && state.turn === 'flee') {
    leaveBattle();
  }
}
...
function resetBones() {
  state.bones = Array.from({ length: 7 }, (_, i) => ({ x: 160 + i * 50, y: -40 - i * 80, w: 10, h: 42, speed: 2.8 }));
}
...
function updateBattle() {
  if (state.turn === 'menu') {
    if (keys.has('ArrowLeft')) state.menuIndex = (state.menuIndex + menuItems.length - 1) % menuItems.length;
    if (keys.has('ArrowRight')) state.menuIndex = (state.menuIndex + 1) % menuItems.length;
  }
...
function drawBattle() {
  ...
  if (state.turn === 'menu') {
    ...
    drawTextBox(['* Froggit was defeated!', '* You won this demo battle. Press X to leave.']);
  } else if (state.turn === 'flee') {
    drawTextBox(['* You spared Froggit.', '* Press X to return to the hallway.']);
  } else if (state.turn === 'lose') {
    drawTextBox(['* You cannot give up just yet...', '* Stay determined. Refresh to retry.']);
  }
